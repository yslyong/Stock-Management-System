...
import { getExpensesByMonth } from "../../Context/DbContext";
...
export default function ExpensesDetails() {
  ...
  let query = useQuery().get("expensesMonth");
  ...
  useEffect(() => {
    ...
    interval = setInterval(() => {
      if (expenses === undefined && count < 1) {
        getExpensesByMonth(query).then((value) => {
          setExpenses(value);
        });
      }
    }, 2000);
  ...
  }, [expenses, count, query]);

  const clickCode = (props) => {
    history.push("/Dashboard/ExpensesSingle?expensesID=" + props);
  };
  ...
  return (
    <>
      <div className="mt-8 space-y-4 py-8 px-4 md:py-12 md:px-8 shadow-xl bg-white ring-2 rounded-lg ring-gray-400 ring-opacity-20">
        <h1 className="lg:text-4xl text-2xl text-center font-bold lg:mb-10 mb-5">Expenses Details for {query}</h1>
        <div>
          <table className="border-collapse w-full">
            <thead className="text-center hidden lg:table-header-group border-2">
              <tr>
                ...
                <th className="p-3 lg:w-4/12 font-bold uppercase bg-gray-200 border text-gray-600 border-gray-300 hidden lg:table-cell">
                  Created Date
                </th>
                <th className="p-3 lg:w-2/12 font-bold uppercase bg-gray-200 border text-gray-600 border-gray-300 hidden lg:table-cell">
                  Total
                  ...
                  >
                    ...
                    <td className="w-full lg:w-auto p-3 text-gray-800 text-center border block lg:table-cell relative lg:static">
                      ...
                      <span
                        ...
                        onClick={() => {
                          clickCode(exp.Expenses_ID);
                        }}
                      >
                        Expenses No {exp.Expenses_ID}
                      </span>
                    </td>
                    <td className="w-full lg:w-auto p-3 text-gray-800 text-center border block lg:table-cell relative lg:static">
                      <span className="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold uppercase">
                    ...
                    <td className="w-full lg:w-auto p-3 text-gray-800 text-center border block lg:table-cell relative lg:static">
                      ...
                      <p> {Moment(exp.Created_Date.toDate()).format("DD/MM/yyyy")}</p>
                      <p> {Moment(exp.Created_Date.toDate()).format("hh:mm A")}</p>
                    </td>
                    <td className="w-full lg:w-auto p-3 text-gray-800 text-center border block lg:table-cell relative lg:static">
                      <span className="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold uppercase">
                        Total
