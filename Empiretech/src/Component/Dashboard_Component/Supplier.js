...
import { getSupplier } from "../../Context/DbContext";
...
export default function Supplier() {
  ...
  const clickCode = (props) => {
    history.push("/Dashboard/SupplierUpdate?supplierID=" + props);
  };
    ...
    <>
      <div className="mt-8 space-y-4 py-8 px-4 md:py-12 md:px-8 shadow-xl bg-white ring-2 rounded-lg ring-gray-400 ring-opacity-20 w-full">
        {location.pathname !== "/Dashboard/Config/" && (
          <h1 className="lg:text-4xl text-2xl text-center font-bold lg:mb-10 mb-5">Supplier Details</h1>
        )}
        <Link to="/Dashboard/SupplierCreate">
          <button
            type="submit"
                ...
                {supplier.map((sup, idx) => (
                  ...
                  >
                    ...
                    <td className="w-full lg:w-auto p-3 pt-6 lg:pt-3 text-gray-800 text-center border block lg:table-cell relative lg:static">
                      ...
                      <span className="pt-10 lg:pt-0">{sup.Supplier_Address}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
